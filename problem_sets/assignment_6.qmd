...
dataset5_after_cleaning \|\> mutate (PM2.5 = as.numeric(PM2.5), time=as.POSIXct(paste(date, hour), format= "%Y-%m-%d %H:%M")) \|\> filter (format(date, "%m") == "09", as.numeric(format(date, "%d")) \<= 7) \|\> ggplot () + geom_line (mapping=aes(x=time, y=PM2.5))
